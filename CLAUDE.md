...
- The group header is three controls and each does one thing: the title renames, the icon beside it collapses, and a delete appears out in the ✕ column while the name is being edited, only enabled once every task in the group is done. Tapping a title must never collapse it. The icon sits against the title rather than at the end of the row, so the total is the last thing on the line and lands over the prices.
...
- Tapping a task takes the caret with it, to the end of what the task says. Without the focus the field never blurs, so the row never commits and never leaves edit mode. Escape therefore has to put the text back **before** it drops the field — removing a focused field blurs it, and the blur commits.
