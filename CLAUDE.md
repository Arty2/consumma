...
- **SHARE and COPY hand over one payload** carrying the link and the code together; either half alone is useless. It goes in `navigator.share`'s `text` with no `url` field, so no target can keep one and drop the other. The link stays bare. Two lines, link then code, with no title and no prose — the code has to be grabbable on its own.
