...
- The group total sits on the header row between the title and the `[…]`, and it is what is **still to buy**: done does not count, half counts in full. **Each row counts as count × price** — three potatoes at 20,00 is 60,00 — while the row itself keeps showing what one costs, because that is what was written down and what gets checked against a shelf. A row with no count is one of the thing, not none of it. Its separator follows whichever mark the counted rows used most, a tie going to the dot; it shows decimals only if a counted price had them; it carries a currency mark only when every counted row that wrote one wrote the same one, after the number. Nothing to total, nothing shown — and it stays while the group is collapsed, which is when it is worth most.
