...
- **Loose ends is drawn as `- - -`, with no rule under it and no control on it.** It is assembled on read, so there is nothing there to rename, delete or carry, and a word set like every other title would offer all three — the rule especially, since a rule is what a title is written on. It is a `p` rather than a button for the same reason: a control that cannot do anything is a tap that goes nowhere and a stop on the way to one. `LOOSE_ENDS_TITLE` stays "Loose ends" and is still the accessible name and what a move is announced as; only the mark changes. It keeps its collapse icon, which is the one thing it can do.
