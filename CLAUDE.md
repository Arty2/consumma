...
- Collapsed state is local, never synced. So is the theme, which DELETE does not clear either — removing a list from a phone is not a change of mind about how the phone should look.
...
- **Nothing is on the page but the sheet and the buttons in its corners.** A burger at the top right opening a side menu that holds sync, share, join, IMPORT/EXPORT/CLEAR/DELETE and the credit; the theme immediately to its left; and sync alone at the top left, which is the one that comes and goes. No opening scroll — nothing sits above the sheet. The row is laid out with `margin-left: auto` on the right-hand pair rather than `space-between`, because the sync button is absent more often than not and space-between with one child pushes it the wrong way.
- **The theme is three states on one button: dark, light, and following the phone.** The first tap goes to the opposite of whatever the phone is doing, then to the one that agrees with it, then back to following — a fixed dark-then-light order makes the first tap on a dark phone do nothing visible, which reads as a broken button. The cycle is therefore computed against the phone and turns around with it. The glyph reports the state it is in, not the one a tap would give. Going back to following removes the key rather than writing `system` into it: arriving writes nothing, and returning to the default has nothing to say either.
...
- **A code is the address of a list on the server, so it does not exist before the first sync.** `sync.load()` never makes one; `sync()` makes it and writes it down before the request, because a PUT that lands while the confirming read fails would otherwise strand a list under a code nobody kept. Until then the menu shows no code and no SHARE or COPY, and the DELETE confirm says there is nothing to come back with.
